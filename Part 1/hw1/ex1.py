...
class DroneProblem(search.Problem):
    ...
    def actions(self, state):
        ...
        for drone in drones:
            ...
            # Pick up packages if they are on the same tile as the package
            # drone can pick-up up to two packages
            if len(drone[1][1]) == 2:
                continue
            ...
            # Deliver a package to a client if the drone has the package and is on the same tile as the client.
            for client in clients:
                # client_loc: index dor the cur tile in the client path
            ...
            # Wait. Does not change anything about the drone.
            if drone[1][1] == ():
    ...
    # turn the tuple of the states to tree lists
        ...
        for act in action:
            ...
            if act[0] == "pick up":
                for drone in drones:
                    if drone[0] == act[1]:
                        Pack = list(drone[1][1])
                        # todo : check if the package doesn't exist
                ...
                for pack in packages:
                    if act[2] == pack[0]:
                        # if the package picked , it put on the second please the word "picked"
                        if pack[1][1] == ('picked',):
                            break
                        # else put the word "picked" on the second place
                        picked = ("picked",)
                        picked = [picked]
            ...
            if node.parent.parent is not None:
                for drone in drones:
                    for dronep in node.parent.parent.state[0]:
                        if dronep[0] == drone[0]:
                            if dronep[1][0] == drone[1][0]:
                                distance = distance + 400
                            else:
                                distance = distance - 100
        for drone in drones:
            for pack in packages:
                distance = distance + (10 * (self.ManhattanDistance(drone[1][0], pack[1][0])))

        for client in clients:
            for pack in packages:
                distance = distance + (10 * (self.ManhattanDistance(client[2][client[3]], pack[1][0])))
        ...
        if node.action is not None:
            for act in node.action:
                if act is not None:
                    ...
                    if act[0] == 'move':
                        distance -= 50
        distance = distance + (len(packages) * 90)
